...
<div {{ $attributes->class('block') }}>
    ...
    <el-select
        ...
        @if ($submitOnChange) onchange="this.closest('form')?.submit()" @endif
    >
        <button type="button" class="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:focus-visible:outline-indigo-500">
            <el-selectedcontent class="col-start-1 row-start-1 flex w-full pr-6 {{ $isStacked ? 'items-start gap-3' : 'gap-2' }}">
                @if ($selectedOption && $selectedOption['icon'] === 'globe')
                    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" class="mt-1 size-4 shrink-0 text-gray-500 dark:text-gray-400">
                        <path d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z" />
                        <path d="M2 10h16M10 2c2 2.2 3 4.8 3 8s-1 5.8-3 8c-2-2.2-3-4.8-3-8s1-5.8 3-8Z" />
                    </svg>
                @elseif ($selectedOption && $selectedOption['swatch'] !== '')
                    <span
                        class="mt-1 inline-flex size-3 shrink-0 rounded-full ring-1 ring-black/10"
                        style="background-color: {{ $selectedOption['swatch'] }}"
                    ></span>
                @endif

                <span class="{{ $isStacked ? 'min-w-0' : 'contents' }}">
                    <span class="block truncate">{{ $selectedOption['label'] ?? $placeholder }}</span>
                    @if (($selectedOption['secondary'] ?? null) || $placeholderSecondary)
                        <span class="{{ $isStacked ? 'mt-0.5 block text-xs' : 'truncate' }} text-gray-500 dark:text-gray-400">{{ $selectedOption['secondary'] ?? $placeholderSecondary }}</span>
                    @endif
                </span>
            </el-selectedcontent>
                ...
                >
                    <div class="flex items-start gap-3">
                        @if ($option['icon'] === 'globe')
                            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" class="mt-1 size-4 shrink-0 text-gray-500 group-focus/option:text-indigo-200 dark:text-gray-400 dark:group-focus/option:text-indigo-100">
                                <path d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z" />
                                <path d="M2 10h16M10 2c2 2.2 3 4.8 3 8s-1 5.8-3 8c-2-2.2-3-4.8-3-8s1-5.8 3-8Z" />
                            </svg>
                        @elseif ($option['swatch'] !== '')
                            <span
                                class="mt-1 inline-flex size-3 shrink-0 rounded-full ring-1 ring-black/10"
                        ...
                        <span class="{{ $isStacked ? 'min-w-0' : 'contents' }}">
                            ...
                            @if ($option['secondary'] !== '')
                                <span class="{{ $isStacked ? 'mt-0.5 block text-xs' : 'ml-2 truncate' }} text-gray-500 group-focus/option:text-indigo-200 dark:text-gray-400 dark:group-focus/option:text-indigo-100">{{ $option['secondary'] }}</span>
                            @endif
                        </span>
                    </div>
