@foreach ($transactions as $transaction)
    <tr class="align-top hover:bg-gray-50">
        ...
        <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-right text-sm font-medium tabular-nums text-emerald-700 dark:border-white/10">
            {{ $transaction->amount_m !== null ? number_format((float) $transaction->amount_m, 2, ',', ' ') : '' }}
        </td>
        <td class="whitespace-nowrap border-b border-gray-200 px-3 py-4 text-right text-sm font-medium tabular-nums text-rose-700 dark:border-white/10">
